...
<?php if (isset($items['products'][0])): ?>
    <?php foreach ($items['products'] as $item): ?>
        ...
        <?php
        $productId = isset($items['relate'][$item->mpn][0]->product_id) ? $items['relate'][$item->mpn][0]->product_id : null;
        ?>
            ...
            <article class="product-card<?= $classEmptyColor; ?>">
                ...
                <section class="product-card__body">
                    <section
                        ...
                        <a class="product-card__img-link<?= $item->compare ? ' is-compare-selected' : '' ?>"
                           data-product-id="<?= $item->product_id ?>"
                           data-sizes='<?= ProductHelper::getSizesJson($item->sizes) ?>'
                           href="/<?= isset($item->url->keyword) ? $item->url->keyword : '#' ?>" target="_blank"">
                        <img data-lazy="<?= ProductHelper::correctedImgPath($item->image) ?>"
                    ...
                    <div class="product-card-bot-view-2">
                        <section class="product-card__prices">
                            ...
                            <?php if ($discount->isExist()) : ?>
                                <section
                                        class="product-card__price product-card__price--new<?= (!$item->stock_status) ? ' gray' : '' ?>">
