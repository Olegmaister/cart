...
/* @var \core\services\cart\PresentItem[] $presents**/
/* @var \core\services\cart\PresentItem $present**/
/* @var \common\entities\Products\Product $productPresent**/
...
<section class="col-lg-6 order-lg-last mb-5 js-wrapper-order-cart">
    <div class="sticky-cart">
        ...
        <div class="checkout-promo promo-form">
            <div class="promo-form__field">
                <input type="text" name="promo-code" value="" class="js-promo-data-input promo-form__input" placeholder="<?= Yii::t('app', 'Enter promo code') ?>">
            </div>
        ...
        <div class="checkout-total-price">
            <div class="checkout-total-price__discount checkout-total-price__col">
                ...
                $discountMoney = $cart->getCost()->getDiscount();

                //dd($cart->getCost()->getDiscounts());

                //получение скидки (%)
                $discountPercent = $cart->getCost()->getDiscountPercent($discountMoney);
                ?>
                <span class="title-h3 title--black"><?= Yii::t('app', 'discount') ?>:</span>
                    ...
                    <span class="js-discount-percent">
						...
						<?php
						if ($discountMoney) {
							echo "<span class='title-h5'>{$currency->getCurrencySign()}</span><span class='js-discount-money'>{$currency->getPrice($discountMoney)}</span>";
						}
						?>
					</span>
            </div>
            <div class="checkout-total-price__delivery checkout-total-price__col">
                ...
                <span class="title-h3 title--red">
					...
					<span class="js-order-cart-delivery-cost"></span>
				</span>
            </div>
            <?php
            $cost = $cart->getCost();
            $discount = $cart->getCost()->getDiscounts();
            ?>
            <div class="checkout-total-price__sum checkout-total-price__col">
                <span class="title-h3 title--black"><?= Yii::t('app', 'sum') ?>: </span>
                <span class="title-h3 title--red">
            <span class="title-h5"><?= $currency->getCurrencySign() ?></span><span data-cost="<?=$currency->getPrice($cost->getTotal() + $cart->getPresentPrice())?>" class="js-cart-popup-cost-total title--red title-h3"><?= $currency->getPrice($cost->getTotal() + $cart->getPresentPrice()) ?></span>
        </span>
            </div>
        </div>
    </div>
